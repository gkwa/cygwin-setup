...
Public Const Version = "6.0.2223.0"
...
class Logging
	...
	Public Function CopyLog()
		...
		For each sLog in Array("Scanstate.log", "USMTCapture.log", "Loadstate.log", "USMTRestore.log", "USMTEstimate.log", "ZTIBackup_imagex.log", "LTIApply_imagex.log", _
			...
			"SMSTS.LOG", "Debug\Netsetup.log", "wpeinit.log", "PnpEnum.xml", "ZTIUserState_xcopy.log", "Debug\DCPROMO.LOG", "Debug\DCPROMOUI.LOG", "OSDSetupWizard.log")
...
Class Utility
	...
	Function RunCommandLog( oExec, fExternalHook, iLimit )
		...
		do while oExec.Status = 0
			...
			If iMinutes >= iLimit and iLimit <> -1 and VarType(iLimit) = 2 then
				...
				RunCommandLog = False
				exit function
			Elseif ( iMinutes \ iHeartBeat ) > LastDiff then
	...
	Function FindFile(sFilename, sFoundPath)
		...
		If oEnvironment.Item("DeployRoot") <> "" then
			...
			For each sDir in Array("\", "\Tools\", "\Servicing\", "\Scripts\", "\Templates\", "\Control\", "\x86\", "\amd64\")
	...
	Function GetOSTargetDriveLetterEx( bRequired )
		...
		Else
			...
			If  oEnvironment.Item("OSDTargetDrive") <> "" Then
